...
- `endGame()` cancels `animId`, but when it is reached from inside `loop` (via `lockPiece` → `spawn`), the loop body afterwards schedules another frame — so the animation keeps running after Game Over even though `gameOver` blocks input. Touch this path carefully.
